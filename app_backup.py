...
def ensure_student_accounts():
    ...
    print(f"ℹ Skipped {skipped_count} accounts (already exists or missing roll)")
...
def allocate_common_subjects_optimized(timetables, branches, years, semesters):
    ...
    for semester in semesters:
        ...
        if not common_subjects:
            print(f"ℹ No common subjects found for semester {semester}")
            continue
...
def preload_subjects():
    ...
    if added > 0:
        ...
        print(f"ℹ All subjects already present")
...
def activate_all_subjects():
    ...
    with app.app_context():
        ...
        if activated_count > 0:
            ...
            print("ℹ All subjects are already active")
...
def load_students_from_files(data_dir="data"):
    ...
    if not files:
        print("ℹ No student files found in data directory")
        return
...
def initialize_data():
    ...
    with app.app_context():
        ...
        if subject_count == 0 or not admin_exists:
            ...
            print(f"ℹ Database already has data: {subject_count} subjects, {Student.query.count()} students")
...
def fix_student_accounts():
    ...
    for student in students:
        try:
            ...
            if not existing_user:
                ...
                result += f"<p>ℹ Already exists: {student.roll}</p>"
    ...
    if created_count > 0:
        ...
        result += "<h4>ℹ No new accounts created (all already exist or errors)</h4>"
