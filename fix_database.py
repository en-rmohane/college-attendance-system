...
def ensure_student_accounts():
    ...
    students = Student.query.filter(Student.roll.isnot(None)).all()  # Ensure roll exists
    created_count = 0
    skipped_count = 0

    for student in students:

        # Validation: Ensure roll exists properly
...
def get_working_days_for_month(year, month):
    ...
    while current_date <= last_day:
        # Monday=0, Friday=4 (0-4 means Monday to Friday)
...
def generate_smart_timetable(branches, years, semesters):
    """Generate clash-free timetable using existing professors"""
    ...
    for branch in branches:
        for year in years:
            for semester in semesters:
                branch_subjects = Subject.query.join(
                    ProfessorSubject,
    ...
    faculty_workload = {prof.id: [] for prof in professors}
    faculty_daily_periods = {prof.id: {day: set() for day in WORKING_DAYS.keys()} for prof in professors}
    timetables = {}
    ...
    for subject in subjects:
        ...
        if academic_year not in years or subject.semester not in semesters:
            continue

        allotment = ProfessorSubject.query.filter_by(subject_id=subject.id).first()
        if not allotment:
            continue
        ...
        lectures_needed = scheme.lectures_per_week if scheme else 3
        lectures_assigned = 0
        max_attempts = 100

        while lectures_assigned < lectures_needed and max_attempts > 0:
            max_attempts -= 1

            if is_professor_overloaded(professor.id, faculty_daily_periods):
                break

            day = random.choice(list(WORKING_DAYS.keys()))
            period = random.choice(list(COLLEGE_TIMINGS.keys()))

            key = f"{branch}_{academic_year}_{subject.semester}"

            if (key in timetables and
                    is_slot_available(timetables[key], day, period) and
                ...
                faculty_workload[professor.id].append({
                    'branch': branch,
                    'year': academic_year,
                    'semester': subject.semester,
                    'day': day,
...
def save_timetable_to_db(timetables):
    ...
    try:
        for key in timetables.keys():
            branch, year, semester = key.split('_')
        ...
        for key, timetable_data in timetables.items():
            ...
            for day_num, day_data in timetable_data['slots'].items():
                for period_num, slot_data in day_data.items():
                    if slot_data['subject'] and slot_data['faculty']:
                        slot = TimetableSlot(
                            branch=branch,
                            year=int(year),
...
def get_timetable_from_db(branch, year, semester):
    ...
    try:
        ...
        for day_num in WORKING_DAYS.keys():
            ...
            for period in range(1, 7):
                timetable['slots'][day_num][period] = {
                    'subject': None, 'faculty': None, 'room': None, 'slot_type': 'lecture'
                }

        for slot in slots:
            if slot.day_of_week in timetable['slots']:
                timetable['slots'][slot.day_of_week][slot.period_number] = {
...
def initialize_data():
    ...
    with app.app_context():
        ...
        if subject_count == 0 or not admin_exists:
            ...
            load_students_from_files()
            migrate_test_system()  # Ensure test system is migrated
...
def student_tests():
    ...
    for test in tests:
        print(f"   - {test.title} | Status: {test.status} | Active: {test.is_active} | Available: {test.available_from} to {test.available_until}")
